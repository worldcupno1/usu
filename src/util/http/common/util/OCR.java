...
public class OCR {
	...
	private static final String apiKey = util.http.common.util.PropertiesUtil.getProperty("OCR.key");
	...
	public static String ocrCode(String imgFilePath, int limitCodeLen){
		...
		try {
			...
			String[] results = util.http.common.util.StringUtil.regex("<Result>([^<]*)</Result>\\s*<Status>([^<]*)</Status>", html);
			if(results.length>0){
